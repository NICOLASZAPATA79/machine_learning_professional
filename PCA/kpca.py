...
if __name__ == '__main__':
    ...
    # Configuración algoritmo Kernel PCA, el número de componentes por defecto es = min(n_registers/n_features)
    ...
    explained_variance = np.var(kpca_x_train,axis=0)
    print(explained_variance)
    explained_variance_ratio = (explained_variance /np.sum(explained_variance))
    print(explained_variance_ratio)
    cum_var = np.cumsum(np.round(explained_variance_ratio,decimals=4)*100)
    components = list(range(0,4))
    ...
    plt.title('Kernel PCA Decomposition')
    ...
    kpca_data_standard = pd.DataFrame(kpca_x_train)
    print(kpca_data_standard)
    print(kpca_data_standard.shape)

    # Clasificación con regresión logistica -PCA

    model = LogisticRegression(solver='lbfgs')
    model.fit(kpca_x_train,y_train)
    y_predict_kpca = model.predict(kpca_x_test)
    print(accuracy_score(y_test,y_predict_kpca))
